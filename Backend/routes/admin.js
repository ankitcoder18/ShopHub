 ...
 *   get:
 *     summary: Get all pending seller requests
 *     tags: [Admin]
 *     security:
 *       - bearerAuth: []
 *     responses:
 ...
 *   put:
 *     summary: Approve a seller request
 *     tags: [Admin]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *   put:
 *     summary: Reject a seller request
 *     tags: [Admin]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *   post:
 *     summary: Create a new superadmin (SuperAdmin only)
 *     tags: [Admin]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *       403:
 *         description: Cannot modify superadmin
 */
// Update user (name, email, phone, role)
router.put('/users/:id', protect, admin, async (req, res) => {
    try {
        const { name, email, phone, role } = req.body;
        const allowedRoles = ['user', 'seller', 'admin']; // only superadmin can assign superadmin
        const update = { };
...
// Delete user (prevent deleting superadmin)
...
router.get('/users/:id/activity', protect, admin, async (req, res) => {
    try {
        const { limit = 50 } = req.query;
        const logs = await Activity.find({ user: req.params.id })
            .sort({ createdAt: -1 })
...
router.get('/users/:id/orders', protect, admin, async (req, res) => {
    try {
        ...
        const orders = await Order.find({ user: req.params.id })
            .populate('seller', 'name email role sellerInfo.businessName')
            .sort({ createdAt: -1 })
            .limit(Math.min(Number(limit) || 50, 200));
...
// Get user purchase summary (totals, counts, sellers involved)
router.get('/users/:id/summary', protect, admin, async (req, res) => {
    try {
        const [orders, user] = await Promise.all([
            Order.find({ user: req.params.id }),
            User.findById(req.params.id).select('name email role createdAt')
        ]);
        if (!user) return res.status(404).json({ success: false, message: 'User not found' });

        const totalOrders = orders.length;
        const delivered = orders.filter(o => o.orderStatus === 'Delivered' || !!o.deliveredAt).length;
        const cancelled = orders.filter(o => o.orderStatus === 'Cancelled' || !!o.cancelledAt).length;
        const totalSpent = orders.reduce((sum, o) => sum + (o.totalPrice || 0), 0);
        const lastOrderAt = orders.sort((a, b) => b.createdAt - a.createdAt)[0]?.createdAt || null;
        const sellers = Array.from(new Set(orders.map(o => String(o.seller || '')))).filter(Boolean);
        ...
        res.status(500).json({ success: false, message: 'Error building user summary', error: error.message });
    }
});
